    ...
    for (const config of playlistConfigs) {
      ...
      if (!playlist) {
        ...
        if (existing) {
          ...
          // Add to index so subsequent playlists in this run can see it
          spotifyPlaylistByName.set(name, { id: spotifyId, name, uri: spotifyUri })
          await prisma.spotifyAccountPlaylist.create({ data: { spotifyId, name, uri: spotifyUri } })
        ...
        try {
          ...
          if (!yearPlaylist) {
            ...
            if (existing) {
              ...
              await prisma.spotifyAccountPlaylist.create({ data: { spotifyId, name: yearName, uri: spotifyUri } })
            }
